...
{    
    ...
    /* add the nex symbol_t to symbol table */
...
#define CHECK_PARSE_ERR(_r,_s) do { \
if(_r == PARSE_ERR) { line->type = TYPE_ERR; err_print(_s); goto out;} \
} while (0);
...
parse_t parse_symbol(char **ptr, char **name)
{
	...
	int len;
	for(len = 0 ; IS_LETTER(*ptr + len) || IS_DIGIT(*ptr + len); len++);
	n = (char *)malloc(len + 1);
	int i;
...
parse_t parse_digit(char **ptr, long *value)
{
	...
	char* check;
	int i = IS_HEX(*ptr) ? 16 : 10;
	*value = strtoll(*ptr, &check, i);
	*ptr = check;
...
#define SET_Y86BIN_CODES(_i) y86bin->codes[_i] = lval & 0xFF; \
			     y86bin->codes[_i+1] = (lval >> 8) & 0xFF; \
			     y86bin->codes[_i+2] = (lval >> 16) & 0xFF; \
			     y86bin->codes[_i+3] = (lval >> 24) & 0xFF;
...
cont:
	...
	char* sbl;
	regid_t reg;
	ret = parse_imm(&cur, &sbl, &lval);
	CHECK_PARSE_ERR(ret, "Invalid Immediate");

	int retemp = parse_delim(&cur, ',');
	CHECK_PARSE_ERR(retemp, "Invalid ','");

	retemp = parse_reg(&cur, &reg);
	CHECK_PARSE_ERR(retemp, "Invalid REG");

	y86bin->codes[1] = HPACK(0xF, reg);
	if (ret == PARSE_DIGIT) {
		SET_Y86BIN_CODES(2);
	}
	else if (ret == PARSE_SYMBOL)
		add_reloc(sbl, y86bin);
	goto cont;
      }
	...
	if (ret == PARSE_DIGIT)
      ...
      case I_DIRECTIVE: {
		...
		if (ret == PARSE_ERR) {
			char* sbl;
			ret = parse_symbol(&cur, &sbl);
			CHECK_PARSE_ERR(ret, "Invalid DATA");
			add_reloc(sbl, y86bin);
		}
...
int relocate(void)
{
	...
	while (rtmp) {
		...
		if (itype == I_IRMOVL) 
			...
			p = (long *)rtmp->y86bin->codes; //I_DER
...
int binfile(FILE *out)
{
	...
	{	//printf("%d %d\n", p->y86bin.addr, HIGH(p->y86bin.codes[0]));
		if(pos < tmp->y86bin.addr && !is_end(tmp)) //fill blanks
		{
			int diff = tmp->y86bin.addr - pos;
			int i;
			for(i = 0; i < diff; i++) fprintf(out, "%c", 0);
				pos = tmp->y86bin.addr;
		}
		size_t num = fwrite(tmp->y86bin.codes, sizeof(byte_t), tmp->y86bin.bytes, out);
		if(num != tmp->y86bin.bytes) 
			return -1;
